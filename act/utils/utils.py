...
def get_norm_stats(dataset_dir, num_episodes, policy_config):
    ...
    # clipping
    left_states_std = torch.clip(left_states_std, 1e-2, np.inf)
    right_states_std = torch.clip(right_states_std, 1e-2, np.inf)
...
def load_data(
    ...
    train_ratio = 0.8  # train-eval: 8-2
    shuffled_indices = np.random.permutation(num_episodes)  # shuffle
    ...
    # obtain normalization stats for eef and action  return mean & std
    norm_stats = get_norm_stats(dataset_dir, num_episodes, policy_config)

    # construct dataset and dataloader normalization
