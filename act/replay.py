...
def robot_action(ros_operator, args, action, action_base, action_velocity):
    # 验证动作是否有效（借鉴inference.py的方式）
    ...
    if args.use_base:
        ros_operator.set_robot_base_target(np.concatenate([action_base, action_velocity]))
...
def init_robot(ros_operator, use_base):
    ...
    input("Enter any key to continue (robot ready):")
...
def main(args):
    ...
    for idx in range(len(replay_actions)):
        print(f'Frame {idx+1}/{len(replay_actions)}')
        robot_action(ros_operator, args, replay_actions[idx], action_base[idx], actions_velocity[idx])
        rate.sleep()
