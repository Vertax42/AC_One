...
def load_latest_checkpoint(ckpt_dir, num_epochs, seed, policy, optimizer):
    ...
    for last_history_epoch in range(num_epochs - 2, -1, -1):
        ckpt_path = os.path.join(
            ckpt_dir,
            f"policy_epoch{last_history_epoch + 1}_seed{seed}_pretrained_all_info.ckpt",
        )

        if os.path.exists(ckpt_path):
            print(
                f"Loading history-trained weights from epoch {last_history_epoch + 1}"
            )
...
def forward_pass(policy_config, data, policy):
    ...
    image_depth_data = (
        image_depth_data.cuda() if policy_config["use_depth_image"] else None
    )
...
def train_process(train_dataloader, val_dataloader, config, stats):
    ...
    tb_log_dir = os.path.join(
        ckpt_dir,
        f"tensorboard_seed{seed}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}",
    )
    writer = SummaryWriter(log_dir=tb_log_dir)

    # 如果需要加载预训练权重
    if pretrain_ckpt == "-1":
        start_epoch, min_val_loss, train_history, validation_history = (
            load_latest_checkpoint(ckpt_dir, num_epochs, seed, policy, optimizer)
        )
    ...
    for epoch in tqdm(range(start_epoch, num_epochs)):
        # 验证模型
        epoch_val_loss, epoch_val_summary = validate_model(
            val_dataloader, policy, policy_config
        )
        ...
        if epoch_val_loss < min_val_loss:
            ...
            save_checkpoint(
                policy, ckpt_dir, ckpt_name, epoch, is_best=True, min_epoch=550
            )

        # 训练模型
        epoch_train_summary = train_epoch(
            train_dataloader, policy, optimizer, policy_config
        )
        ...
        if epoch != 0 and epoch % 500 == 0:
            save_checkpoint(
                policy, ckpt_dir, f"policy_epoch_{epoch}_seed_{seed}.ckpt", epoch
            )
        ...
        if (
            epoch != 0
            and datasets_reload_each_epoch != 0
            and epoch % datasets_reload_each_epoch == 0
        ):
    ...
    final_ckpt_path = os.path.join(
        ckpt_dir, f"policy_epoch{epoch + 1}_seed{seed}_pretrained_all_info.ckpt"
    )
    checkpoint = {
    ...
    save_checkpoint(
        best_state_dict,
        ckpt_dir,
        f"policy_epoch_{best_epoch}_seed_{seed}.ckpt",
        best_epoch,
    )
...
def parse_args(known=False):
    ...
    parser.add_argument(
        "--lr_backbone", type=float, default=4e-5, help="learning rate for backbone"
    )
    parser.add_argument(
    ...
    # 图像设置
    parser.add_argument(
        "--use_depth_image", action="store_true", help="use depth images"
    )

    # 状态和动作设置
    parser.add_argument(
        "--arm_delay_time", type=int, default=0, help="arm delay time in milliseconds"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--use_eef_action", action="store_true", help="use eef data for actions"
    )
