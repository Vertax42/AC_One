...
main() {
    ...
    log_info ""
    log_info "下一步："
    log_info "0. 使用 ./00_eef_control_ros_openpi.sh 进行末端位置控制"
    log_info "0. 使用 ./00_joint_control_ros_openpi.sh 进行关节空间控制"
    log_info "0. 使用 ./00_pico_xr_teleop_ros_openpi.sh 进行pico xr遥控器控制"
