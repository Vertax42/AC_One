...
# 00_pico_xr_control_ros_openpi.sh - PICO XR遥操作控制脚本 (ros_openpi环境版本)
...
cleanup_terminals() {
    ...
    pkill -f "pico_xr" 2>/dev/null || true
    sleep 1

    # 2. 关闭终端窗口 - 更精确的匹配，避免误关闭Cursor
    local titles=("ac_one" "can1" "can3" "can6" "joy" "🎮 PICO XR Node" "🛡️ PICO XR Teleop")
...
main() {
    ...
    log_info "🎮 开始启动PICO XR遥操作控制系统 (ros_openpi环境)..."
    ...
    # 等待CAN接口就绪
    log_info "等待CAN接口就绪..."
    check_can_ready "can1" || { log_error "CAN1接口启动失败"; log_warn "CAN1接口启动异常，但继续执行，但可能遇到错误..."; }
    check_can_ready "can3" || { log_error "CAN3接口启动失败"; log_warn "CAN3接口启动异常，但继续执行，但可能遇到错误..."; }
    check_can_ready "can6" || { log_error "CAN6接口启动失败"; log_warn "CAN6接口启动异常，但继续执行，但可能遇到错误..."; }

    # 启动ROS2系统 (末端控制模式)
    log_step "启动ROS2机械臂控制器 (末端控制模式)..."
    gnome-terminal --title="ac_one" -x $shell_type -i -c "
        ...
        echo \"🤖 启动机械臂末端控制器...\";
        ros2 launch arx_x5_controller v2_eef_control.launch.py;
        $shell_exec"
    sleep 3
    ...
    # 启动PICO XR遥操作系统 (在分离的终端中)
    log_step "启动PICO XR遥操作系统..."
    gnome-terminal --title="pico_xr_teleop" -x $shell_type -i -c "
        eval \"\$(mamba shell hook --shell bash)\";
        mamba activate ros_openpi;
        cd ${workspace}/../../ROS2/X5_ws;
        source install/setup.bash;
        ...
        echo \"🛡️ 安全提示：\";
        echo \"  - 观察安全状态显示\";
        echo \"  - 看到 '🟢 SAFE' 和 '✅ Ready to grip!' 时可按grip按键\";
        echo \"  - 松开grip按键立即停止控制\";
        echo \"======================================\";
        ros2 launch pico_xr_teleop pico_xr_teleop.launch.py;
        $shell_exec"
    sleep 3
    ...
    # 检查节点状态
    log_info "检查系统节点状态..."
    check_ros2_node_fast "/arm_eef_slave_l" || { log_warn "左臂末端控制器节点启动异常"; }
    check_ros2_node_fast "/arm_eef_slave_r" || { log_warn "右臂末端控制器节点启动异常"; }
    check_ros2_node_fast "arx_joy" || { log_warn "手柄节点启动异常"; }
    check_ros2_node_fast "pico_xr_node" || { log_warn "PICO XR节点启动异常"; }
    check_ros2_node_fast "pico_xr_teleop_node" || { log_warn "PICO XR遥操作节点启动异常"; }

    log_info "🎮 PICO XR遥操作控制系统启动完成！"
    log_info "环境信息:"
    ...
    echo ""
    log_info "🎮 PICO XR遥操作控制说明："
    log_info "1. 观察PICO XR遥操作终端的安全状态显示"
    log_info "2. 看到 '🟢 SAFE' 和 '✅ Ready to grip!' 时可以按grip按键开始遥操作"
    log_info "3. VR控制器的移动和旋转会实时传递给机械臂"
    log_info "4. 松开grip按键立即停止遥操作"
    log_info "5. 系统具备多层安全保护机制"
    log_info ""
    log_info "🛡️ 安全特性："
    log_info "  - RPY角度范围限制: ±90度"
    log_info "  - 姿态差异安全检查: ±17.2度"
    log_info "  - 实时安全状态监控"
    log_info "  - 相对控制，避免跳变"
    log_info ""
    log_info "终端窗口说明："
    log_info "  - ac_one: 机械臂控制器"
    log_info "  - joy: 手柄控制"
    log_info "  - 🎮 PICO XR Node: VR控制器数据"
    log_info "  - 🛡️ PICO XR Teleop: 安全监控和遥操作控制"
    log_info ""
    log_info "清理说明："
